...
public class MybatisController {
	...
	public void setSqlSession(SqlSession sqlSession) {
		...
		System.out.println("마이바티스 사용준비 끝");
	}
	...
	public String list(Model model, HttpServletRequest req) {
		...
		/*
		현재 페이지에 출력할 게시물을 인출한다. 이때 앞에서 계산한
	...
	public ModelAndView loginAction(HttpServletRequest req,
		...
		if(dto==null) {
			...
			mv.addObject("LoginNG", "아디디/패스워드가 틀렸습니다.");
			//로그인 페이지를 화면에 출력한다.
	...
	public String writeAction(Model model, HttpServletRequest req,
		...
		호출한다. 이때 아이디의 경우 세션영역에 저장되어 있으므로 형변환
		후 getter()를 통해 아이디를 읽어온다.
	...
	public String modify(Model model, HttpServletRequest req,
		...
		확인했지만, 필요하다면 작성자 본인이 맞는지까지 확인하면 더욱좋다.
		 */
		if(session.getAttribute("siteUserInfo")==null) {
		...
		//인터페이스를 통해 Mapper의 view 메서드를 호출한다. 이떄
	...
	public String modifyAction(HttpSession session,
		...
		//방명록 게시판은 상세보기 페이지가 없으므로 수정완료시 리스트로
		//이동하면 된다.
		return "redirect:list.do";
	}
}
